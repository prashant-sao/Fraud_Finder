...
def analyze_text():
    """Endpoint to analyze job posting text directly"""
    try:
        data = request.get_json()
        ...
        # Analyze for fraud
        analysis = detector.analyze_job_posting(text, url)
...
def home():
    ...
    return jsonify({
        'service': 'Job Fraud Detection API',
        'version': '1.0',
        'endpoints': {
            'POST /api/analyze': 'Analyze job posting from URL',
            'POST /api/analyze-text': 'Analyze job posting text directly',
...
class JobFraudDetector:
    ...
    def analyze_job_posting(self, content, url):
        ...
        content_lower = content.lower()
        fraud_score = 0
        red_flags = {}  # Changed from list to dict
        details = {}
